...
#[derive(Deserialize)]
pub struct BadgeVersionForRepoQueryParams {
    #[serde(rename = "header")]
...
#[derive(FromRow)]
pub struct BadgeVersionForRepoPackage {
    pub version: String,
    pub status: PackageStatus,
    pub flags: i32,
}

impl PackageWithVersion for BadgeVersionForRepoPackage {
    fn version(&self) -> &str {
        &self.version
    }
}
impl PackageWithFlags for BadgeVersionForRepoPackage {
    fn flags(&self) -> PackageFlags {
        PackageFlags::from_bits(self.flags as u32).expect("flags must be deserializable")
    }
}
impl PackageWithStatus for BadgeVersionForRepoPackage {
...
pub async fn badge_version_for_repo(
    ...
    State(state): State<AppState>,
    Query(query): Query<BadgeVersionForRepoQueryParams>,
) -> EndpointResult {
    let project_name = if let Some(project_name) = project_name.strip_suffix(".svg") {
        project_name
    } else {
    ...
    let packages: Vec<BadgeVersionForRepoPackage> = sqlx::query_as(
        r#"
        SELECT
            version,
