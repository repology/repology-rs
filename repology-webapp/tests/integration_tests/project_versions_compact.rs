...
async fn test_normal(pool: PgPool) {
    ...
    assert_eq!(response.header_value_str("content-type").unwrap(), Some("text/html"));
    assert!(response.is_html_valid(HtmlValidationFlags::ALLOW_EMPTY_TAGS | HtmlValidationFlags::WARNINGS_ARE_FATAL));
    assert!(response.text().unwrap().contains("Versions (compact view) for <strong>zsh"));
...
async fn test_redirect(pool: PgPool) {
    let response = Request::new(pool, "/project/zsh-old/versions-compact").perform().await;
    assert_eq!(response.status(), http::StatusCode::MOVED_PERMANENTLY);
    assert_eq!(response.header_value_str("location").unwrap(), Some("/project/zsh/versions-compact"));
