            ...
            SELECT * FROM (
                (
                    SELECT
                        ts AS timestamp,
                        CASE
                            WHEN (snapshot->$1->>'num_metapackages_newest')::integer + (snapshot->$1->>'num_metapackages_outdated')::integer = 0 THEN NULL
                            ELSE 100.0::real * (snapshot->$1->>$2)::real / ((snapshot->$1->>'num_metapackages_newest')::real + (snapshot->$1->>'num_metapackages_outdated')::real)
                        END AS value
