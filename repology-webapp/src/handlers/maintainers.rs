...
pub async fn maintainers(
    ...
    maintainers_generic(&my_route, None, None, query, &state).await
}
...
pub async fn maintainers_bounded(
    ...
    if let Some(end) = bound.strip_prefix("..") {
        maintainers_generic(&my_route, None, Some(end), query, &state).await
    } else {
        maintainers_generic(&my_route, Some(&bound), None, query, &state).await
    }
}
