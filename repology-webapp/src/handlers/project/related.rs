...
use crate::handlers::projects::common::CategorizedDisplayVersions;
use crate::handlers::projects::common::PackageForListing;
use crate::handlers::projects::common::packages_to_categorized_display_versions_per_project;
...
pub async fn project_related(
    ...
    let Some(project) = project else {
        return nonexisting_project(&my_route, &state, &cookies, project_name, None).await;
    };

    if project.is_orphaned() {
        return nonexisting_project(&my_route, &state, &cookies, project_name, Some(project)).await;
    }
